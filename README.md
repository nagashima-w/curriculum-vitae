...
- 展示会に出すデモのための基盤構築を実施
- 試験的にGitHub Actionsを採用してDocker ImageのCI環境を整備した
...
- オンプレミスのGPUサーバー群で構成されている社内の機械学習基盤が抱える問題の課題解決を行っている
- 以下のような問題がある
  - OSとGPUドライバーのバージョンが古い
  - 学習環境へ道的なGPUアサインができない
- AI開発の担当者と協力しながら以下の取り組みを行っており、2021年中での完了を目指している
  - データサイエンスのフェーズとAIを用いたサービス開発のフェーズのそれぞれに応じた最適な学習環境の構築方法の調査と実装
  - パブリッククラウドも活用した、ハイブリッドクラウドを用いた学習環境の模索
  - 新環境に移行後の監視方法の調査
